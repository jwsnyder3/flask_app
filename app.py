...
s3_client = boto3.client(
    's3',
    aws_access_key_id=os.getenv('AWS_ACCESS_KEY_ID'),  # Load from environment
    aws_secret_access_key=os.getenv('AWS_SECRET_ACCESS_KEY'),  # Load from environment
    region_name=os.getenv('AWS_REGION')  # Load from environment
)

S3_BUCKET_NAME = os.getenv('S3_BUCKET_NAME')
bucket = f"https://{S3_BUCKET_NAME}.s3.{os.getenv('AWS_REGION')}.amazonaws.com/"
...
def covers():
    try:
        ...
        return render_template("covers.html", covers=covers, bucket=bucket)
...
def bookstore():
    ...
    return render_template('bookstore.html', books=books_with_inventory_and_specials, first_name=first_name, bucket=bucket)
...
def index():
    ...
    return render_template('login.html', bucket=bucket)  # Render the login page
...
def admin():
    ...
    return render_template('admin.html', first_name=first_name, contacts=contacts, bucket=bucket)

@app.route('/home')
def home():
    return render_template('bookstore.html', bucket=bucket)
...
def search():
    ...
    return render_template('search.html', books=books_with_specials, genres=genres, first_name=first_name, bucket=bucket)

@app.route('/about')
def about():
    return render_template('about.html', bucket=bucket)
...
def contact():
    ...
    return render_template('contact.html', bucket=bucket)
...
def respond_to_contact(contact_id):
    ...
    return render_template('respond.html', contact=contact, bucket=bucket)
...
def admin_contact():
    ...
    return render_template('admin_contact.html', contacts=contacts, bucket=bucket)
...
def account():
    ...
    return render_template('account.html', user=user, address=address, bucket=bucket)
...
def order_history():
    ...
    return render_template('order_history.html', order_history=order_history, bucket=bucket)
...
def reviews():
    ...
    return render_template('reviews.html', reviews=reviews, bucket=bucket)
...
def contact_messages():
    ...
    return render_template('contact_messages.html', contacts=contacts, bucket=bucket)
...
def cart():
    ...
    return render_template('cart.html', cart_items=cart_items, total_price=total_price, bucket=bucket)
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        cursor.close()
        return render_template('checkout.html', saved_addresses=saved_addresses, cart_items=cart_items, total_price=total_price, bucket=bucket)
...
def order_confirmation():
    ...
    cursor.close()
    return render_template('order_confirmation.html', order=order, order_items=order_items, total_price=total_price, bucket=bucket)
