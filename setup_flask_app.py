...
"""
    ...
    # Paths to the Nginx configuration
    nginx_config_path = "/etc/nginx/sites-available/flask_app"
    symlink_path = "/etc/nginx/sites-enabled/flask_app"

    # Write the Nginx config file to a temporary file
    temp_file_path = "flask_app_nginx"
    with open(temp_file_path, "w") as temp_file:
        temp_file.write(nginx_config)

    # Move the config file to the sites-available directory and create a symlink in sites-enabled
...
def configure_ufw():
    ...
    try:
        ...
        run_command("sudo ufw allow 'Nginx Full'")  # Nginx for Gunicorn Flask
...
def install_gunicorn():
    ...
    run_command("pip install gunicorn", use_venv=True)
